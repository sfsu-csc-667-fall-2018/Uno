...
function getCreateDatabaseQuery(sequelize, config, options) {
  switch (config.dialect) {
    ...
    case 'mssql':
      return 'IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = \'', config.database, '\')' + 'BEGIN' + 'CREATE DATABASE ' + sequelize.getQueryInterface().escape(config.database) + (options.collate ? ' COLLATE ' + sequelize.getQueryInterface().escape(options.collate) : '') + 'END;';
      break;
